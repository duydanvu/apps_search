...
class SearchController extends Controller
{
    ...
    public function searchProductTestPerformance($keyword = 'and', $local = 'en',$shopId = 7){
        $dataSearchQueries = DB::table('productstests')
            ->select('productstests.product_id','title','description','locales','image',
                \DB::raw("MAX(variants.price) AS max_price"),
                \DB::raw("MIN(variants.price) AS min_price"))
            ->join('productstestdetails','productstestdetails.product_id','=','productstests.product_id')
            ->join('variants','variants.product_id','=','productstests.product_id')
            ->where('locales',$local)
            ->where('productstests.shop_id',$shopId)
            ->where(function ($query) use ($keyword){
                $query  ->where('title','like','%'.$keyword.'%')
                    ->orwhere('description','like','%'.$keyword.'%');
            })->groupBy('product_id')
            ->groupBy('title','handle','image','description','locales')
            ->get();
        return response() -> json(['success' =>true,'data'=>$dataSearchQueries]);
    }

    public function searchProductTestPerformance1($keyword = 'and', $local = 'en',$shopId = 7){
        $dataSearchQueries = DB::table('products')
        ->select('products.product_id','title','handle','description','image','locales',
        \DB::raw("MAX(variants.price) AS max_price"),
        \DB::raw("MIN(variants.price) AS min_price"))
